  ...
  const getStatusClass = (status: ReservationStatus) => {
    const statusClasses: Record<ReservationStatus, string> = {
      [ReservationStatus.PENDING]:
        "px-3 py-1 rounded-xl text-sm font-semibold bg-[#fff3cd] text-[#856404]",
      [ReservationStatus.CONFIRMED]:
        "px-3 py-1 rounded-xl text-sm font-semibold bg-[#cfe2ff] text-[#084298]",
      [ReservationStatus.ACTIVE]:
        "px-3 py-1 rounded-xl text-sm font-semibold bg-[#d1e7dd] text-[#0f5132]",
      [ReservationStatus.COMPLETED]:
        "px-3 py-1 rounded-xl text-sm font-semibold bg-[#d1e7dd] text-[#0f5132]",
      [ReservationStatus.CANCELLED]:
        "px-3 py-1 rounded-xl text-sm font-semibold bg-[#f8d7da] text-[#842029]",
    };
      ...
      <div className="bg-card p-8 border border-border rounded-lg shadow-md print:shadow-none print:border-none">
        ...
        <div className="grid grid-cols-2 gap-8 pb-8 border-b-2 border-border mb-8">
          <div>
            ...
            <p className="m-2.5 mt-0 text-muted-foreground">
              خدمات اجاره خودرو
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-8 py-6 mb-8 border-b border-border">
          <div>
            <h3 className="text-lg m-0 mb-4 text-foreground border-b-2 border-border pb-2">
              اطلاعات مشتری
            </h3>
            ...
            <p className="m-3 mt-0 text-muted-foreground leading-relaxed">
              <strong className="text-foreground">تلفن:</strong>{" "}
              {reservation.phone}
            </p>
          </div>
          ...
          <div>
            ...
            <p className="m-3 mt-0 text-muted-foreground leading-relaxed">
              ...
              <span
                className={getPaymentStatusClass(reservation.paymentStatus)}
              >
        ...
        <div className="my-8">
          ...
          <table className="w-full border-collapse mb-8">
            <thead className="bg-muted">
              <tr>
                <th className="p-4 text-right font-semibold text-foreground border-b-2 border-border">
                  نام خودرو
                </th>
                <th className="p-4 text-right font-semibold text-foreground border-b-2 border-border">
                  روزهای اجاره
              ...
              <tr>
                <td className="p-4 border-b border-border text-muted-foreground">
                  <div>
                    ...
                    <p className="text-muted-foreground text-sm m-1 mt-0">
                      {rental.car.model}
                    </p>
                    {rental.withDriver && (
                      <span className="inline-block px-2.5 py-1 bg-primary/10 text-primary rounded text-xs mt-1">
                        راننده شامل
                      </span>
                    )}
                  </div>
                ...
                <td className="p-4 border-b border-border text-muted-foreground">
                  {rental.pricePerDay.toLocaleString("fa-IR")} تومان
                </td>
                <td className="p-4 border-b border-border text-muted-foreground">
                  {rental.rentalDays}
                </td>
        ...
        {rental.selectedOptions.length > 0 && (
          <div className="bg-muted p-6 rounded-md my-8">
            <h3 className="mt-0 text-lg mb-4 text-foreground">
              خدمات و اپشن‌های انتخاب شده
            </h3>
        ...
        <div className="text-center pt-8 border-t border-border text-muted-foreground text-sm">
          ...
          <p className="m-2.5 mt-0">
            تلفن پشتیبانی: 021-1234-5678 | ایمیل: support@caremon.ir
          </p>
        </div>
      </div>
    </div>
